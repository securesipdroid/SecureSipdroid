...
import org.sipdroid.sipuas.ui.Settings;
...
public class IpAddress {
	...
	public static void setLocalIpAddress() {
		...
		try {
			String preferredInterfaceName = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_PREFERRED_IFACE, Settings.DEFAULT_PREFERRED_IFACE);
			Log.d("Sipdroid-IpAddress-setLocalIpAddress", "Preferred interface: " + preferredInterfaceName);
			NetworkInterface preferredInterface = NetworkInterface.getByName(preferredInterfaceName);
			for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements();) {
				NetworkInterface intf = en.nextElement();
				Log.d("Sipdroid-IpAddress-setLocalIpAddress", "Checking interface: " + intf.getName());
				
				if(preferredInterfaceName.equals(Settings.PREF_PREFERRED_IFACE)
						|| preferredInterface == null
						|| intf.getName().equals(preferredInterfaceName)){
					for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements();) {
						...
						if (!inetAddress.isLoopbackAddress()) { 
							if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(Settings.PREF_STUN, Settings.DEFAULT_STUN)) {
								...
								Log.d("Sipdroid-IpAddress-setLocalIpAddress", "Using IP: " + localIpAddress);
							} else {
								try {
