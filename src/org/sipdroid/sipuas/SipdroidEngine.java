...
package org.sipdroid.sipuas;

import java.io.IOException;
import java.net.UnknownHostException;

import org.sipdroid.net.KeepAliveSip;
import org.sipdroid.sipuas.ui.ChangeAccount;
import org.sipdroid.sipuas.ui.LoopAlarm;
...
public class SipdroidEngine implements RegisterAgentListener {
	...
	public boolean StartEngine() {
			...
			if (wl == null) {
				if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).contains(org.sipdroid.sipuas.ui.Settings.PREF_KEEPON)) {
					Editor edit = PreferenceManager.getDefaultSharedPreferences(getUIContext()).edit();
	
					edit.putBoolean(org.sipdroid.sipuas.ui.Settings.PREF_KEEPON, Build.MODEL.equals("Nexus One") ||
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					...
					if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(org.sipdroid.sipuas.ui.Settings.PREF_KEEPON, org.sipdroid.sipuas.ui.Settings.DEFAULT_KEEPON))
						pwl[i] = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "Sipdroid.SipdroidEngine");
				}
				
				try {
					SipStack.debug_level = 0;
		//			SipStack.log_path = "/data/data/org.sipdroid.sipuas";
